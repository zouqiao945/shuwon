...
class Webuploader extends InputWidget{
    ...
    public function init()
    {
        ...
        $this->options['innerHTML'] = isset($this->options['innerHTML']) ? $this->options['innerHTML'] : '<button class="btn btn-default">选择文件</button>';
    ...
    public function run()
    {
        ...
        $content = $value ?
        Html::tag(
            'img', '', ['src'=>strpos($value, 'http:') === false ? ('/admin/img/file.jpg') : $value,'width'=>140,'height'=>140])
        :
        $this->options['innerHTML'];

        if($this->hasModel()){
            return Html::tag('div', $content, ['id'=>$this->options['boxId']]) . Html::activeHiddenInput($this->model, $this->attribute).( $value ? "<a onclick='$(this).prev().val(\"\");$(this).prev().prev().find(\"img\").attr(\"src\",\"\");'>删除文件</a>" : "");
        }else{
...
uploader.on( 'uploadSuccess', function( file, data ) {
    ...
    $( '#{$this->options['boxId']} .webuploader-pick' ).html('<video src="/admin/img/file.jpg" width="140" height="140"/>');
...
uploader.on( 'uploadProgress', function( file, percentage ) {
    ...
    uploader.on( 'uploadSuccess', function( file, data ) {
        var url = data.key;
        $( '#'+file.id ).find('p.state').text('上传成功').fadeOut();
        $( '#{$this->options['boxId']} .webuploader-pick' ).html('<img src="/admin/img/file.jpg" width="140" height="140"/>');
