...
class Webuploader extends InputWidget{
    ...
    public function init()
    {
        ...
        $this->options['innerHTML'] = isset($this->options['innerHTML']) ? $this->options['innerHTML'] : '<button class="btn btn-default">选择视频</button>';
    ...
    public function run()
    {
        ...
        $content = $value ?
        Html::tag(
            'video', '', ['src'=>strpos($value, 'http:') === false ? (\Yii::getAlias('@static') . '/' . $value) : $value,'width'=>$this->options['previewWidth'],'height'=>$this->options['previewHeight']])
        :
        $this->options['innerHTML'];

        if($this->hasModel()){
            return Html::tag('div', $content, ['id'=>$this->options['boxId']]) . Html::activeHiddenInput($this->model, $this->attribute).( $value ? "<a onclick='$(this).prev().val(\"\");$(this).prev().prev().find(\"video\").attr(\"src\",\"\");'>删除视频</a>" : "");
        }else{
...
uploader.on( 'uploadSuccess', function( file, data ) {
    ...
    $( '#{$this->options['boxId']} .webuploader-pick' ).html('<video src="{$web}/'+data.url+'" width="{$this->options['previewWidth']}" height="{$this->options['previewHeight']}"/>');
...
uploader.on( 'uploadProgress', function( file, percentage ) {
    ...
    uploader.on( 'uploadSuccess', function( file, data ) {
        var url = data.key;
        $( '#'+file.id ).find('p.state').text('上传成功').fadeOut();
        $( '#{$this->options['boxId']} .webuploader-pick' ).html('<video src="{$this->domain}'+url+'" width="{$this->options['previewWidth']}" height="{$this->options['previewHeight']}"/>');
